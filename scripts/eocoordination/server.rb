  ...
  module Coordination
    module Transport
      ...
      class Server
        ...
        # @param host [String]
        # @param port [Integer]
        # @param registry [#upsert, #remove, #snapshot, nil] optional route owner
        ...
        def start
          @mutex.synchronize do
            ...
            @thread = @accept_thread_factory.call do
              Lich.log("info: EO Coordination accept thread started pid=#{Process.pid} port=#{@port}") if defined?(Lich) && Lich.respond_to?(:log)
              accept_loop
            end
          end
          true
        ...
        def accept_loop
          loop do
            ...
            unless server
              unless stopping?
                Lich.log("warning: EO Coordination accept_loop exiting: @server is nil pid=#{Process.pid}") if defined?(Lich) && Lich.respond_to?(:log)
              end
              break
            end
            ...
            begin
              ...
              Lich.log("warning: EO Coordination accept_loop closed unexpectedly: #{e.class} pid=#{Process.pid}") if !stopping? && defined?(Lich) && Lich.respond_to?(:log)
              break
            rescue StandardError => e
              socket&.close rescue nil
              Lich.log("warning: EO Coordination accept_loop error (continuing): #{e.class}: #{e.message}") if defined?(Lich) && Lich.respond_to?(:log)
            end
          end
        rescue StandardError => e
          Lich.log("error: EO Coordination accept_loop fatal: #{e.class}: #{e.message}\n\t#{e.backtrace&.first(5)&.join("\n\t")}") if defined?(Lich) && Lich.respond_to?(:log)
        ensure
          if defined?(Lich) && Lich.respond_to?(:log)
            if stopping?
              Lich.log("info: EO Coordination accept_loop stopped pid=#{Process.pid}")
            else
              Lich.log("warning: EO Coordination accept_loop thread exiting pid=#{Process.pid}")
            end
          end
        end
        ...
        def handle_client(socket)
          ...
          unless raw
            Lich.log('warning: EO Coordination client read timed out') if defined?(Lich) && Lich.respond_to?(:log)
            return
          end
        ...
        def unauthorized_response
          Lich.log('warning: EO Coordination unauthorized local request rejected') if defined?(Lich) && Lich.respond_to?(:log)
