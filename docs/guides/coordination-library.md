...
Lich core remains responsible for facts that only its parser can establish and
for native script lifecycle and execution guards. In particular, a coherent
player observation must come from Lich's parser-owned `Game.player_state`
publisher; a library cannot reconstruct that provenance by sampling `XMLData`
and `GameObj` independently.
...
The library validates source bindings but cannot create coherence itself; that
must be supplied by the native observation publisher.
