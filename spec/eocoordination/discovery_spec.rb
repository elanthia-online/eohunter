...
require_relative '../../scripts/eocoordination/discovery'
require 'fileutils'
require 'tmpdir'

class CoordinationDiscoveryRegistry
  def initialize
    @records = {}
  end

  def upsert(payload)
    pid = payload.fetch(:pid)
    @records[pid] = (@records[pid] || {}).merge(payload)
  end
  ...
  def snapshot
    { source: 'ActiveSessionsAPI', sessions: @records.values }
  end
end

RSpec.describe EO::Coordination::Discovery do
  let(:coordination) { EO::Coordination }
  ...
  let(:registry) { CoordinationDiscoveryRegistry.new }
  ...
  context 'with native registry and endpoint transports' do
    ...
    before do
      root = ENV['LICH_ROOT']
      skip 'set LICH_ROOT to run native ActiveSessions integration' unless root

      require File.join(root, 'lib/internal_api/active_sessions')
    ...
    after do
      @endpoint&.close
      native.stop_service! if defined?(Lich::InternalAPI::ActiveSessions)
