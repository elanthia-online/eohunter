...
RSpec.describe EO::Coordination do
  let(:now) { [100.0] }
  let(:session) do
  ...
  it 'does not create a listener or threads by default' do
    expect(EO::Coordination::Transport::Server).not_to receive(:new)
  ...
  it 'reuses the bounded native transport and advertises no credential' do
    server = instance_double(EO::Coordination::Transport::Server, start: true, running?: true,
                             host: '127.0.0.1', port: 34_567, stop: nil)
    expect(EO::Coordination::Transport::Server).to receive(:new).with(
  ...
  it 'serves only authenticated read snapshots over the reused loopback transport', :real_socket do
    ...
    raw = EO::Coordination::Transport::Client.new(
